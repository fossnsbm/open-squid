...
const PhaseSection: React.FC = () => {
    ...
    const handlePrev = () => setCurrent((prev) => (prev === 0 ? cards.length - 1 : prev - 1));
    const handleNext = () => setCurrent((prev) => (prev === cards.length - 1 ? 0 : prev + 1));
            ...
            <div className="w-full flex flex-col items-center justify-center">
              ...
              <div className="flex justify-center mt-4 space-x-2">
                {cards.map((_, idx) => (
                  <span
                    key={idx}
                    className={`inline-block w-2 h-2 rounded-full ${idx === current ? "bg-pink-600" : "bg-gray-300"}`}
                  />
                ))}
              </div>
            </div>
